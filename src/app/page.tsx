...
void loop() {
  ...
  return (
    <div className="flex h-screen flex-col bg-[#07090e] text-zinc-100 font-sans antialiased overflow-hidden">
      {/* 1. Header Studio */}
      <header className="flex h-14 items-center justify-between border-b border-zinc-800 bg-[#090d16] px-6">
        <div className="flex items-center gap-3">
          <div className="relative flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-tr from-cyan-600 to-indigo-600 text-white font-bold shadow-lg shadow-cyan-500/20">
            P
          ...
          <div>
            ...
            <p className="text-[10px] text-zinc-500 font-mono">WORKSPACE // OFFLINE_MODE</p>
          </div>
        </div>

        {/* Mode Toggle */}
        <div className="flex items-center rounded-lg bg-[#0e1322] p-1 border border-zinc-800">
          <button
            onClick={() => setMode("schematic")}
            className={`flex items-center gap-2 rounded px-3 py-1 text-xs font-medium transition-all ${
              mode === "schematic"
                ? "bg-gradient-to-r from-cyan-600 to-cyan-500 text-white shadow-md shadow-cyan-500/10"
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg border border-emerald-900/50 bg-emerald-950/15 hover:bg-emerald-950/25 px-3 py-1.5 text-xs font-semibold text-emerald-400 transition"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg border border-zinc-800 bg-zinc-900/50 hover:bg-zinc-800/80 hover:border-zinc-700 px-3 py-1.5 text-xs font-semibold text-zinc-300 transition"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg border border-zinc-800 bg-zinc-900/50 hover:bg-zinc-800/80 hover:border-zinc-700 px-3.5 py-1.5 text-xs font-medium text-zinc-300 transition"
          >
          ...
          <button
            onClick={clearBoard}
            className="flex items-center gap-1.5 rounded-lg border border-zinc-800 bg-zinc-900/50 hover:bg-red-950/20 hover:border-red-900/30 px-3.5 py-1.5 text-xs font-medium text-zinc-400 hover:text-red-400 transition"
          >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* 2. Left Panel: Component Library & Tool Box */}
        <aside className="w-80 border-r border-zinc-800 bg-[#090d16] flex flex-col overflow-hidden">
          {/* Tool Box Selectors */}
          <div className="p-4 border-b border-zinc-800">
            <h2 className="text-xs font-semibold tracking-wider text-zinc-400 mb-3 font-mono">TOOLS</h2>
            ...
            {activeTool === "component" && (
              <div className="mt-3 rounded-lg bg-cyan-950/20 border border-cyan-900/30 p-2.5 text-[11px] text-cyan-400/90 leading-relaxed flex gap-2">
                <Info className="h-4 w-4 flex-shrink-0 mt-0.5 text-cyan-400" />
                <span>Select a component below and click on the canvas to place it.</span>
              </div>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
            <h2 className="text-xs font-semibold tracking-wider text-zinc-400 mb-3 font-mono">COMPONENT LIBRARY</h2>
            <div className="flex flex-col gap-2.5">
              {Object.entries(COMPONENT_LIBRARY).map(([key, item]) => (
                <div
                  key={key}
                  onClick={() => {
                  ...
                  className={`group relative rounded-xl border p-3 cursor-pointer transition flex flex-col ${
                    selectedLibraryRef === key
                      ? "bg-cyan-950/20 border-cyan-500/80 shadow-md shadow-cyan-950/50"
                      : "bg-[#0b101b] border-zinc-800/80 hover:border-zinc-700/80 hover:bg-zinc-900/10"
                  }`}
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-semibold tracking-wide text-zinc-200 group-hover:text-cyan-400 transition font-mono">
                      {item.name}
                    </span>
                    <span className="rounded bg-zinc-800/80 px-1.5 py-0.5 text-[9px] text-zinc-400 uppercase font-mono">
                      {item.category}
                    </span>
                  </div>
                  ...
                  <div className="mt-2.5 flex items-center justify-between text-[9px] text-zinc-500">
                    ...
                    <span className="text-zinc-600 font-mono">{item.width}x{item.height} mm</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <main className="flex-1 p-4 bg-[#05070a] flex flex-col overflow-hidden">
          <CanvasArea mode={mode} />
        </main>

        {/* 4. Right Panel: Properties & Design Rules */}
        <aside className="w-80 border-l border-zinc-800 bg-[#090d16] flex flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
            {selectedComponent ? (
              // Component Properties Panel
              <div className="flex flex-col gap-5">
                <div>
                  <h2 className="text-xs font-semibold tracking-wider text-zinc-400 font-mono mb-1">PROPERTIES</h2>
                  <h3 className="text-sm font-bold text-zinc-100 flex items-center gap-1.5">
                    <Cpu className="h-4 w-4 text-cyan-400" />
                    {selectedComponent.libraryRef}
                  </h3>
                  <span className="text-[10px] text-zinc-500 font-mono uppercase tracking-wider">{selectedComponent.id}</span>
                </div>

                {/* Footprint Specifications */}
                <div className="rounded-xl border border-zinc-800 bg-[#0b101b] p-3.5 flex flex-col gap-2.5">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 rounded-lg border border-zinc-800 bg-[#0e1322] hover:bg-zinc-800/80 px-3 py-2 text-xs font-semibold transition"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 rounded-lg border border-red-950/40 bg-red-950/10 hover:bg-red-950/30 hover:border-red-900/50 px-3 py-2 text-xs font-semibold text-red-400 transition"
                  >
                    <Trash2 className="h-3.5 w-3.5" />
                    Delete Component
                  </button>
                ...
                <div>
                  <h3 className="text-xs font-semibold tracking-wider text-zinc-400 font-mono mb-2.5">PINS & NETLIST ({selectedComponent.pins.length})</h3>
                    ...
                    {selectedComponent.pins.map((pin) => (
                      ...
                      >
                        <div className="flex items-center gap-1.5">
                          <span className="inline-block w-5 rounded bg-zinc-800 text-center text-[9px] font-bold text-zinc-400 font-mono">
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`text-[10px] font-mono rounded px-1.5 py-0.5 ${
                            pin.connectedNetId 
                              ...
                              : "bg-zinc-800/40 text-zinc-500 border border-zinc-800/20"
                          }`}>
                            {pin.connectedNetId 
                          ...
                          {wireStartPin?.compId === selectedComponent.id && wireStartPin?.pinId === pin.pinId ? (
                            <button
                              ...
                              className="flex items-center justify-center h-6 w-6 rounded bg-emerald-500 hover:bg-emerald-600 text-white font-bold animate-pulse cursor-pointer"
                            >
                            ...
                            <button
                              ...
                              className="flex items-center justify-center h-6 w-6 rounded border border-zinc-800 bg-[#0e1322] hover:bg-zinc-800 text-zinc-400 hover:text-cyan-400 transition cursor-pointer"
                            >
                              <Zap className="h-3 w-3" />
                            </button>
                          )}
              ...
              <div className="flex flex-col gap-5">
                <div>
                  <h2 className="text-xs font-semibold tracking-wider text-zinc-400 font-mono mb-1">NET PROPERTIES</h2>
                  ...
                  <span className="text-[10px] text-zinc-500 font-mono uppercase tracking-wider">{selectedNetId}</span>
                </div>
                ...
                <div className="flex flex-col gap-2 font-mono">
                  {boardState.nets[selectedNetId].segments.length > 0 && (
                    <button
                      ...
                      className="flex items-center justify-center gap-1.5 rounded-lg border border-yellow-950/40 bg-yellow-950/10 hover:bg-yellow-950/30 hover:border-yellow-900/50 px-3 py-2 text-xs font-semibold text-yellow-400 transition cursor-pointer"
                    >
                      <RotateCw className="h-3.5 w-3.5" />
                      Clear Copper Traces
                    </button>
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-1.5 rounded-lg border border-red-950/40 bg-red-950/10 hover:bg-red-950/30 hover:border-red-900/50 px-3 py-2 text-xs font-semibold text-red-400 transition cursor-pointer"
                  >
                    <Trash2 className="h-3.5 w-3.5" />
                    Hapus Seluruh Kabel
                  </button>
                ...
                {/* Pin Connections */}
                <div>
                  <h3 className="text-xs font-semibold tracking-wider text-zinc-400 font-mono mb-2.5">CONNECTED PINS</h3>
                  <div className="flex flex-col gap-1.5 max-h-[160px] overflow-y-auto pr-1">
                    {boardState.nets[selectedNetId].connectedPins.map((pinRef) => {
                      ...
                      return (
                        <div key={pinRef} className="flex items-center justify-between rounded-lg border border-zinc-800 bg-zinc-900/30 p-2 text-xs text-zinc-300 font-mono">
                          <span>{comp ? `${comp.libraryRef.split("_")[0]} (${pinLabel})` : pinRef}</span>
                          <span className="text-[10px] text-zinc-500 uppercase">{compId.split("_")[1] || compId}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                {boardState.nets[selectedNetId].segments.length > 0 && (
                  <div>
                    <h3 className="text-xs font-semibold tracking-wider text-zinc-400 font-mono mb-2.5">COPPER TRACE SEGMENTS</h3>
                    <div className="flex flex-col gap-1.5 max-h-[160px] overflow-y-auto pr-1">
                      {boardState.nets[selectedNetId].segments.map((seg, idx) => (
                        <div key={idx} className="flex items-center justify-between rounded-lg border border-zinc-800 bg-zinc-900/30 p-2 text-xs">
                          ...
                          <button
                            ...
                            className="p-1 rounded hover:bg-red-950/20 hover:text-red-400 text-zinc-500 transition cursor-pointer"
                          >
                            <Trash2 className="h-3.5 w-3.5" />
                          </button>
                        </div>
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="grid grid-cols-2 gap-3.5">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] text-zinc-500 font-mono uppercase">WIDTH (mm)</label>
                    <input
                      type="number"
                      value={boardWidth}
                      onChange={handleWidthChange}
                      className="rounded-lg border border-zinc-800 bg-[#0b101b] px-3 py-1.5 font-mono text-xs text-zinc-200 focus:border-cyan-500 focus:outline-none transition"
                    />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] text-zinc-500 font-mono uppercase">HEIGHT (mm)</label>
                    <input
                      type="number"
                      value={boardHeight}
                ...
                <div className="border-t border-zinc-800 pt-5">
                  <h3 className="text-xs font-semibold tracking-wider text-zinc-400 font-mono mb-4 flex items-center gap-1.5">
                ...
                {/* DRC Violations List */}
                <div className="border-t border-zinc-800 pt-5">
                  <h3 className="text-xs font-semibold tracking-wider text-zinc-400 font-mono mb-3">
                  ...
                  {drcViolations.length === 0 ? (
                    ...
                    <div className="flex flex-col gap-2 max-h-[180px] overflow-y-auto pr-1">
                      {drcViolations.map((violation) => (
                        <div 
                          key={violation.id} 
                          className="rounded-xl border border-red-950/40 bg-red-950/15 p-3 text-[11px] text-red-400/90 leading-snug flex gap-2"
                        >
