export type LayerType = 'TopCopper' | 'BottomCopper' | 'Silkscreen' | 'Drill';

export interface PinDef {
  pinId: string; // UUID unik pin
  label: string; // Misal: "GND", "VCC", "GPIO5"
  x: number; // Posisi relatif x dari pusat komponen (dalam mm)
  y: number; // Posisi relatif y dari pusat komponen (dalam mm)
...
export interface ComponentNode {
  ...
  libraryRef: string; // Referensi ke library komponen baku (e.g., "ESP32-C3-WROOM")
  position: { x: number; y: number }; // Koordinat x, y absolut di canvas
...
export interface BoardState {
  boardId: string;
  width: number; // Lebar PCB dalam mm
  height: number; // Tinggi PCB dalam mm
  components: Record<string, ComponentNode>;
  nets: Record<string, Net>;
}

export interface DRCSettings {
  minTraceWidth: number; // Minimum lebar trace (misal: 0.15mm / 6mil)
  minClearance: number; // Jarak aman antar trace/pad (misal: 0.15mm / 6mil)
  minViaHoleSize: number; // Diameter lubang Via minimum (misal: 0.3mm)
  minViaDiameter: number; // Diameter luar cincin Via (misal: 0.6mm)
  silkscreenClearance: number; // Jarak silkscreen ke pad tembaga (hindari tumpang tindih)
  // Fitur Khusus: Isolasi Sinyal (USP)
  analogDigitalIsolation: number; // Clearance ekstra antara trace Analog dan Digital (misal: 0.5mm minimum)
}
